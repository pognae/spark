...
/*팝업창 화면 중앙에 띄우기*/
function centerwin(path, w, h, scroll, param){
...
/*모달 팝업창 화면 중앙에 띄우기*/
function centerModalWin(path, w, h, scroll, param) {
	winToTop = (screen.height) ? (screen.height-h)/2 : 0;
	winToLeft = (screen.width) ? (screen.width-w)/2 : 0;
	making = 'dialogWidth='+w+';dialogHeight='+h+';top='+winToTop+';left='+winToLeft+';scrollbars='+scroll+';resizable=0';

    var rtnValue = window.showModalDialog(path, param, making);
	rtnValue.focus();
}
...
function comCodeData(code) {
    doPost({
    	url:"/common/code/gridCodeSearch",
